...
//  state_X_pro=state_X_tmp;
    ...
    CovrMatrix_P_pro=CovrMatrix_tmp;
...
//    Mea_Nosiecovr <<	0.0009, 0.0009, 0.0009,
//                        1, 1, 1,
//                        1, 1, 1;
...
void EKF_Attitude::init(Vector_12 init_measurement,double _currT)
{
    deltaT=_currT;
    state_X_pro=init_measurement;
}
...
void EKF_Attitude::Prior_Predict()
{
  ...
  Vect_to_SkewMat(angular_vel, w_angular_T);  //Cbn
  w_angular = w_angular_T.transpose();    //Cnb
  ...
  if (using_2ndOrder)
    ...
    Matrix_3 Temp_Mat2 = Identity_Matrix3 + w_angular_T*deltaT + deltaT*deltaT/2*w_angular_T*w_angular_T;
  ...
  CovrMatrix_tmp = CovrMatrix_P_pro;
...
Eigen::Quaterniond EKF_Attitude::Run(Vector_9 measurement,double _currT)
{
    deltaT=_currT-deltaT;
	while (true)
	{
